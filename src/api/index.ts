...
import { AlbumByAlbumId, AlbumBySingerId, MusicByAlbumId, MusicByIdArrType, MusicByIdType, MusicBySingerId, MusicIdByAlbumId, OtherAlbum, PlaylistType, RecommendAlbum, SeachResponseType, SingerAllType, SingerById, SingerType, UserLikeMusicType } from './type'
...
// 获取推荐歌单
export const getRecommendPlaylist = (pageNum?:number,pageSize?:number):Promise<PlaylistType> => request.get(`${BASE_URL}/playlist/recommend`,{params:{pageNum,pageSize}})

// 获取推荐专辑
export const getRecommendAlbum = (pageNum?:number,pageSize?):Promise<RecommendAlbum> => request.get(`${BASE_URL}/album/recommend`,{params:{pageNum,pageSize}})
...
// 根据国籍、性别、获取歌手信息
export const getSingerByOther = (initial:string,language:string,gender:string,pageNum:number=1,pageSize:number=12):Promise<SingerAllType> => {
